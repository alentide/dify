...
const CreateAppCard = forwardRef<HTMLAnchorElement>((_, ref) => {
  ...
  return (
    <div
      className="flex flex-col bg-background-default-dimm border-[0.5px] border-components-panel-border rounded-xl
      min-h-[160px] transition-all duration-200 ease-in-out"
    >
      <Link
        ref={ref}
        className="group flex grow items-start p-4 cursor-pointer"
        href="/datasets/create"
      >
        <div className="flex items-center gap-3">
          <div
            className="w-10 h-10 p-2 flex items-center justify-center border border-dashed border-divider-regular rounded-lg
      ...
      <div className="p-4 pt-0 text-text-tertiary system-xs-regular">
        {t('dataset.createDatasetIntro')}
      </div>
      <Link
        className="group flex p-4 items-center gap-1 border-t-[0.5px] border-divider-subtle rounded-b-xl cursor-pointer"
        href="/datasets/connect"
      >
        <div className="system-xs-medium text-text-tertiary group-hover:text-text-accent">
